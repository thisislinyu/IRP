...
set.seed(355)
trainIndex <- createDataPartition(model_dat_82_new$outcome, p = 0.8, list = FALSE)
trainingSet <- model_dat_82_new[trainIndex,]
testSet <- model_dat_82_new[-trainIndex,]

model_dat_82_new$outcome %>% table() ## 48/190=0.2526
...
for (i in 5:15) {
    print(i)
    print(hoslem.test(labels, predictions, g=i)$p.value)
}

cali_dat = data.frame(id = c(1:nrow(testSet)),
                      labels, predictions)
calibration.plot(cali_dat, which.model = 1, na.rm = FALSE, alpha = 0.05, N.bins = 7, 
xlab = "Predicted Probability of Occurrence", 
ylab = "Observed Occurrence as Proportion", 
...
twoClassCtrl <- trainControl(
  ...
  savePredictions = "final",
  classProbs = T,
  summaryFunction = BigSummary, ###prSummary
...
set.seed(355)
glmnet_m <- train(outcome ~., data = trainingSet1, method = "glmnet",  metric = "auc_ap_sbr",  trControl = twoClassCtrl, tuneLength = 20)
...
pred_test_dat <- pred_dat_f(dat = testSet1,outcome = testSet1$outcome,model=glmnet_m)


classes <- c("one", "zero")

train_perf <- BigSummary(pred_train_dat,lev = classes)
test_perf <- BigSummary(pred_test_dat,lev = classes)
...
# pred_train_dat: predicted values for the validation set
# pred_test_dat:predictied values for the test set
# model_name: which model
# the string representation of the outcome variable
...
test_perf <- c(data_type="Test",
               model_name = model_name,
               BigSummary(pred_test_dat,lev = classes))
...
glm_perf <- sum_perf_f(glmnet, model_name='glmnet') %>% data.frame()
...
```{r}
set.seed(355)
glm_pred_dat <- data.frame(fittedProb,testSet1$outcome,Group = "glmnet")
...
```{r}
z = log(predictions/(1-predictions))
logit_newprob = -0.94+z
new_nprob = 1/(1+exp(-logit_newprob ))

fittedProb1 <- fittedProb %>% mutate(
  one = new_nprob,zero = 1-new_nprob
)
...
res_test <- evalm(glm_pred_dat,positive = 'one',bin=5)

glm_pred_dat1 <- data.frame(fittedProb1,testSet1$outcome,Group = "glmnet")

res_test <- evalm(glm_pred_dat,positive = 'one',bin=5)

res_test1 <- evalm(glm_pred_dat1,positive = 'one',bin=5)

tmp <- glmnet_m$results

tmp1 <- glmnet_m$pred

density(tmp1$one) %>% plot()
```

##### matrix for the train
```{r}
glm_pred_train <- glmnet_m$pred
z = log(glm_pred_train$one/(1-glm_pred_train$one))
logit_newprob = -1.14+z
glm_pred_train$one = 1/(1+exp(-logit_newprob ))
glm_pred_train$zero = 1- glm_pred_train$one


glm_pred_train <- glmnet_m$pred
train_glm_dat <- data.frame(one = glm_pred_train$one,
                        zero = glm_pred_train$zero,
                        obs = glm_pred_train$obs)



res <- evalm(glmnet_m,positive = 'one',showplots = TRUE,bins=5)

res_train_recal <- evalm(train_glm_dat,positive = 'one',showplots = TRUE,bins=5)
...
```{r}
cali_dat_recali = train_glm_dat %>% 
  select(obs,one)

cali_dat_recali$id = c(1:nrow(cali_dat_recali))


cali_dat_recali <- cali_dat_recali %>% select(
  id,obs,one
) %>% mutate(obs = c(1,0)[obs])
  
  data.frame(id = c(1:nrow(testSet)),
                      labels,new_nprob )#predictions


calibration.plot(cali_dat_recali, which.model = 1, na.rm = FALSE, alpha = 0.05, N.bins = 5, 
xlab = "Predicted Probability of Occurrence", 
ylab = "Observed Occurrence as Proportion", 
main = NULL, color= NULL, model.names= NULL)
```

#### recalibration for the train
```{r}
res_glm_train <- evalm(glmnet_m,positive = 'one',showplots = TRUE,bins=5)

tmp <- glmnet_m$results

tmp_glm <- glmnet_m$pred
tmp_glm$obs_num <- c(1,0)[tmp_glm$obs]


val.prob.ci.2(tmp_glm$one, tmp_glm$obs_num, CL.smooth = TRUE, logistic.cal = FALSE, lty.log = 2,
 col.log = "red", lwd.log = 1.5)
```
...
```{r}
roc_glmnet <- roc(labels, predictions,smooth =TRUE,ci = TRUE)
roc_glmnet_train <- roc(c(1,0)[glm_pred_train$obs], glm_pred_train$one,smooth =TRUE,ci = TRUE)

g2 <- ggroc(list(Validation=roc_glmnet_train, Test=roc_glmnet), size = 2,legacy.axes=TRUE)+
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), color="black",alpha = 2, linetype="dashed")+
  theme_minimal()
g2
```
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_dat_82[,c("Height")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Temperature")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Smoking")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("KPS score")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Number of underlying diseases")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Previous lung disease")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("ICIs drugs")],
            y = model_dat_82$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_dat_82[,c("ICIs dosage")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Course of treatment")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Number of combined antitumor drugs")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Number of combined drugs")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Previous treatment_yn")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Number of previous treatment drugs")],
            y = model_dat_82$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_dat_82[,c("Percentage of CD4 lymphocytes")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Percentage of NK cell")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Percentage of  T lymphocytes")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Percentage of eosinophilus cells")],
            y = model_dat_82$outcome,
            plot = "box")
...
featurePlot(x = model_dat_82[,c("Percentage of neutrophilic granulocyte")],
            y = model_dat_82$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
nearZeroVar(model_dat_82, freqCut = 95/5, uniqueCut = 10,names = TRUE)
```

...
model_dat1201 <- model_dat %>% 
  mutate(`Number of underlying diseases`=`Number of underlying diseases` %>% as.numeric()) %>% 
  mutate(`Number of combined drugs`=ifelse(is.na(`Number of combined drugs`),median(na.omit(`Number of combined drugs`)),`Number of combined drugs`) %>% as.numeric(),
         
         `Number of previous treatment drugs`=ifelse(is.na(`Number of previous treatment drugs`),median(na.omit(`Number of previous treatment drugs`) %>% as.numeric()),`Number of previous treatment drugs`)%>% as.numeric(),
         
         `Number of combined antitumor drugs`=ifelse(is.na(`Number of combined antitumor drugs`),median(na.omit(`Number of combined antitumor drugs`)),`Number of combined antitumor drugs`)%>% as.numeric(),
...
# model_dat1201$`Number of underlying diseases` =  fct_collapse(model_dat1201$`Number of underlying diseases`,
#                                                    "2" = c("2","3", "4", "5"))
...
# model_dat1201$`Number of previous treatment drugs` =
#   fct_collapse(model_dat1201$`Number of previous treatment drugs`,
#                                                    
#  "1" = c("1","2","3", "4", "5") )
# 
# 
...
# 
# model_dat1201$`Number of combined drugs` =
#   fct_collapse(model_dat1201$`Number of combined drugs`,
#                                                    "1" = c("1", "2","3","4","5","6") 
#   )

## 0为一类；1-4为一类，然后是否可以再分亚组1-2、3-4.

# model_dat1201$`Number of combined antitumor drugs` =
#   fct_collapse(model_dat1201$`Number of combined antitumor drugs`,
#                                                    "1" = c("1","2","3","4") 
#   )
# 
# ## KPS 合并 70 及以下为一类； 80为一类，90及以上为一类
# model_dat1201$`KPS score` =
#   fct_collapse(model_dat1201$`KPS score` %>% as.character,
#                                                    "70" = c("30","40","50","60","70") ,
#  "90" = c("90","100")          
#   )
#   
# 
# miss_var_summary(model_dat1201)

model_dat1201$outcome <- as.factor(model_dat1201$outcome)

```

## Create dummy variable
```{r}
dummyModel <- dummyVars(as.factor(outcome) ~ ., data = model_dat1201)
model_dat1201X <- as.data.frame(predict(dummyModel, newdata = model_dat1201))


rangeModel <- preProcess(model_dat1201X, method = "range")
model_dat1201X <- predict(rangeModel, newdata = model_dat1201X)


model_dat1201_new <- cbind(model_dat1201$outcome, model_dat1201X)
names(model_dat1201_new)[1] <- "outcome"
```
...
set.seed(355)
trainIndex <- createDataPartition(model_dat1201_new$outcome, p = 0.8, list = FALSE)
trainingSet <- model_dat1201_new[trainIndex,]
testSet <- model_dat1201_new[-trainIndex,]

model_dat1201_new$outcome %>% table() ## 48/190=0.2526
...
methodList <- c("rf", "glmnet", "xgbTree",'C5.0Tree')
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_dat1201[,c("Height")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Temperature")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Smoking")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("KPS score")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Number of underlying diseases")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Previous lung disease")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("ICIs drugs")],
            y = model_dat1201$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_dat1201[,c("ICIs dosage")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Course of treatment")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Number of combined antitumor drugs")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Number of combined drugs")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Previous treatment_yn")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Number of previous treatment drugs")],
            y = model_dat1201$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_dat1201[,c("Percentage of CD4 lymphocytes")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Percentage of NK cell")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Percentage of  T lymphocytes")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Percentage of eosinophilus cells")],
            y = model_dat1201$outcome,
            plot = "box")
...
featurePlot(x = model_dat1201[,c("Percentage of neutrophilic granulocyte")],
            y = model_dat1201$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
nearZeroVar(model_dat1201, freqCut = 95/5, uniqueCut = 10,names = TRUE)
```

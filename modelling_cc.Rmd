...
cc <- complete.cases (model_dat) 

model_dat <-  model_dat[cc,]
...
model_datcc$`Number of underlying diseases` =  fct_collapse(model_datcc$`Number of underlying diseases`,
                                                   "2" = c("2","3", "4", "5"))
...
model_datcc$`Number of previous treatment drugs` =
  fct_collapse(model_datcc$`Number of previous treatment drugs`,
...
model_datcc$`Number of combined drugs` =
  fct_collapse(model_datcc$`Number of combined drugs`,
                                                   "1" = c("1", "2","3","4","5","6") 
  )

## 0为一类；1-4为一类，然后是否可以再分亚组1-2、3-4.

model_datcc$`Number of combined antitumor drugs` =
  fct_collapse(model_datcc$`Number of combined antitumor drugs`,
                                                   "1" = c("1","2","3","4") 
  )

## KPS 合并 70 及以下为一类； 80为一类，90及以上为一类
model_datcc$`KPS score` =
  fct_collapse(model_datcc$`KPS score` %>% as.character,
                                                   "70" = c("30","40","50","60","70") ,
 "90" = c("90","100")          
  )
  

miss_var_summary(model_datcc)

model_datcc$outcome <- as.factor(model_datcc$outcome)

```

## Create dummy variable
```{r}
dummyModel <- dummyVars(as.factor(outcome) ~ ., data = model_datcc)
model_datccX <- as.data.frame(predict(dummyModel, newdata = model_datcc))


rangeModel <- preProcess(model_datccX, method = "range")
model_datccX <- predict(rangeModel, newdata = model_datccX)


model_datcc_new <- cbind(model_datcc$outcome, model_datccX)
names(model_datcc_new)[1] <- "outcome"
```
...
set.seed(355)
trainIndex <- createDataPartition(model_datcc_new$outcome, p = 0.7, list = FALSE)
trainingSet <- model_datcc_new[trainIndex,]
testSet <- model_datcc_new[-trainIndex,]

model_datcc_new$outcome %>% table() ## 48/190=0.2526
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_datcc[,c("Height")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Temperature")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Smoking")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("KPS score")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Number of underlying diseases")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Previous lung disease")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("ICIs drugs")],
            y = model_datcc$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_datcc[,c("ICIs dosage")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Course of treatment")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Number of combined antitumor drugs")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Number of combined drugs")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Previous treatment_yn")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Number of previous treatment drugs")],
            y = model_datcc$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
featurePlot(x = model_datcc[,c("Percentage of CD4 lymphocytes")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Percentage of NK cell")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Percentage of  T lymphocytes")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Percentage of eosinophilus cells")],
            y = model_datcc$outcome,
            plot = "box")
...
featurePlot(x = model_datcc[,c("Percentage of neutrophilic granulocyte")],
            y = model_datcc$outcome,
            plot = "box")
...
```{r eval=FALSE, include=FALSE}
nearZeroVar(model_datcc, freqCut = 95/5, uniqueCut = 10,names = TRUE)
```
